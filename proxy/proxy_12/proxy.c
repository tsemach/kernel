...
static irqreturn_t 	proxy_irq(int _irq, void *_dev);
...
static irqreturn_t
proxy_irq(int _irq, void *_dev)
{
	printk(KERN_INFO "proxy interrupt handler is called, irq = %i", _irq);
...
static int __init tsemach_init(void) 
{
	...
	if (proxy_is_irq) {
		rc = request_irq(0, proxy_irq, IRQF_SHARED, "proxy_irq", &proxy_private);
		if (rc != 0) {
			printk(KERN_INFO "ERROR failed register proxy irq %i", 120);
